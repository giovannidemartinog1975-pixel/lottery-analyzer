...
const C = { bg:"#07070F",card:"#0D0D1A",border:"#1A1A2E",text:"#E0E0F0",dim:"#6A6A8A",orange:"#F07030",teal:"#2BA89A",red:"#C94040",purple:"#8A5CC4",green:"#4A9E5C",blue:"#4A8FD4" };
...
function buildStats(draws:Draw[]) {
  ...
  return { total, getRit:(n:number)=>total-1-(last[n]??-1), getFreq:(n:number)=>freq[n]||0 };
}
...
function generateFiltered(
  filters:Filters, st:ReturnType<typeof buildStats>|null,
  ...
  function enrich(nums:number[]):Combo {
    const ev=nums.filter(n=>n%2===0).length;
...
export default function AppGeneratoreAvanzato() {
  ...
  // Statistiche decine e frequenti dal dataset
  const decineStats=useMemo(()=>{
    if(!stats||draws.length===0) return DECINE.map(()=>({media:0,pct:0}));
    return DECINE.map(d=>{
      const media=draws.reduce((s,dr)=>s+dr.nums.filter(n=>n>=d.a&&n<=d.b).length,0)/draws.length;
      return {media,pct:Math.round(media/PICK*100)};
    });
  },[draws,stats]);

  const freqStats=useMemo(()=>{
    if(!stats||draws.length===0) return {hot3pct:0,hot3count:0};
    let hot3count=0;
    for(let n=1;n<=90;n++) if(stats.getFreq(n)>=3) hot3count++;
    return {hot3count,hot3pct:Math.round(hot3count/90*100)};
  },[stats,draws]);
  ...
  const genera=useCallback(()=>{
    ...
    generateFiltered(filters,statsRef.current,
  ...
  if(loadingDraws) return <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}><div style={{color:ACCENT,fontSize:28}}>🎯</div><div style={{color:ACCENT,fontFamily:"Georgia,serif",fontSize:18}}>Caricamento dati storici...</div></div>;
    ...
    <div style={{maxWidth:820,margin:"0 auto",padding:"0 12px"}}>
      ...
      {stats&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(100px,1fr))",gap:8,marginBottom:14}}>
          ...
          <KpiCard label="Numeri caldi" value={freqStats.hot3count} color={C.orange} sub={`≥3x su ${draws.length} est.`}/>
      ...
      <div style={{background:C.card,border:`1px solid ${phase===1?`${ACCENT}66`:C.border}`,borderRadius:12,padding:16,marginBottom:14}}>
        ...
        <div style={{background:"#080816",borderRadius:8,padding:12,marginBottom:12,border:`1px solid ${C.border}`}}>
          ...
          <div style={{color:C.dim,fontSize:10,marginBottom:8}}>La barra mostra la media storica di numeri estratti per decina</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(75px,1fr))",gap:4}}>
            {DECINE.map((d,i)=>{
              const cnt=fDec.get(i)||0;
              const ds=decineStats[i];
              const maxMedia=Math.max(...decineStats.map(x=>x.media));
              return(
                <div key={d.l} style={{background:cnt>0?`${DC[i]}18`:"#0a0a18",border:`2px solid ${cnt>0?DC[i]:C.border}`,borderRadius:8,padding:"5px 3px",textAlign:"center"}}>
                  ...
                  <div style={{background:"#0a0a1a",borderRadius:3,height:4,overflow:"hidden",margin:"3px 2px"}}>
                    <div style={{background:DC[i],height:"100%",width:`${(ds.media/Math.max(maxMedia,0.1))*100}%`}}/>
                  </div>
                  <div style={{color:C.dim,fontSize:8,marginBottom:2}}>μ={ds.media.toFixed(1)}</div>
        ...
        {/* Frequenti + Ritardo con statistiche */}
        {stats&&(
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
            <div style={{background:"#080816",borderRadius:8,padding:12,border:`1px solid ${C.border}`}}>
              ...
              <div style={{color:C.dim,fontSize:9,marginBottom:8}}>{freqStats.hot3count} numeri ≥3x ({freqStats.hot3pct}% del pool)</div>
              <div style={{display:"flex",gap:4}}>
                {[0,1,2,3,4].map(n=>{
                  const pct=n===0?100:Math.round(Math.pow(freqStats.hot3count/90,n)*100);
                  return(
                    <button key={n} onClick={()=>setFMinFreq(n)} style={{flex:1,background:fMinFreq===n?`${C.orange}22`:"transparent",color:fMinFreq===n?C.orange:C.dim,border:`1px solid ${fMinFreq===n?C.orange:C.border}`,borderRadius:6,padding:"5px 2px",fontSize:10,cursor:"pointer",fontFamily:"inherit",textAlign:"center"}}>
                      ...
                      {n>0&&<div style={{fontSize:8,color:C.dim}}>~{pct}%</div>}
                    </button>
                  );
                })}
              </div>
            ...
            <div style={{background:"#080816",borderRadius:8,padding:12,border:`1px solid ${C.border}`}}>
              ...
              <div style={{color:C.dim,fontSize:9,marginBottom:8}}>Ritardo medio dei 6 numeri della sestina</div>
              <div style={{display:"flex",gap:4}}>
        ...
        {generating&&(
          <div style={{marginBottom:12}}>
            <div style={{background:"#0a0a18",borderRadius:4,height:8,overflow:"hidden",marginBottom:4}}>
              <div style={{background:`linear-gradient(90deg,${ACCENT},${C.teal})`,height:"100%",width:`${genPct}%`,transition:"width 0.15s"}}/>
            </div>
