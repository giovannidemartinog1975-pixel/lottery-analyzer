...
// ═══════════════════════════════════════════════════════════════
const DRAWS = [
  { n:28, date:"07/04", nums:[4,12,21,33,48],  stelle:[1,4] },
  { n:29, date:"10/04", nums:[2,17,29,40,46],  stelle:[3,7] },
  { n:30, date:"14/04", nums:[8,19,27,37,50],  stelle:[2,9] },
  { n:31, date:"17/04", nums:[3,15,22,35,44],  stelle:[5,11] },
  { n:32, date:"21/04", nums:[11,23,31,42,49], stelle:[1,8] },
  { n:33, date:"24/04", nums:[5,18,26,39,47],  stelle:[4,10] },
  { n:34, date:"28/04", nums:[7,20,28,41,45],  stelle:[3,12] },
  { n:35, date:"01/05", nums:[1,14,30,38,50],  stelle:[2,6] },
  { n:36, date:"06/05", nums:[9,16,24,36,48],  stelle:[5,9] },
  { n:37, date:"08/05", nums:[6,13,25,40,46],  stelle:[1,7] },
  { n:38, date:"12/05", nums:[4,26,32,35,36],  stelle:[3,11] },
  { n:39, date:"15/05", nums:[3,10,38,41,43],  stelle:[2,5] },
  { n:40, date:"19/05", nums:[2,12,20,38,45],  stelle:[2,5] },
  { n:41, date:"22/05", nums:[6,22,26,31,37],  stelle:[5,8] },
];

const MU_TEO    = 127.5;
const SIGMA_TEO = 30;
const JACKPOT = "76.000.000 CHF";
const ACCENT    = "#4A8FD4";
const POOL      = 50;
const PICK      = 5;
const STELLE_POOL  = 12;
const STELLE_COUNT = 2;
const POPULAR   = new Set([1,2,3,4,5,10,20,30,40,50]);
const LS_KEY_EM = "draws_euromillions_v1";
const LS_TICKETS_EM = "tickets_euromillions_v1";
...
function generateStelle(seed){
  ...
  const pool=Array.from({length:STELLE_POOL},(_,i)=>i+1);
  const picked=[];
...
function calcBonusAffinita(num, allDraws, ticketSum, sigmaRef, bf){
  const freq=allDraws.filter(d=>(d[bf]||[]).includes(num)).length;
  const freqPct=freq/(allDraws.length*2);
  const simDraws=allDraws.filter(d=>Math.abs(sm(d.nums)-ticketSum)<=sigmaRef);
  const coFreq=simDraws.length>0?simDraws.filter(d=>(d[bf]||[]).includes(num)).length/(Math.max(simDraws.length,1)*2):freqPct;
  let lastIdx=-1;
  for(let i=allDraws.length-1;i>=0;i--){if((allDraws[i][bf]||[]).includes(num)){lastIdx=i;break;}}
...
function getBonusSuggestions(allDraws, ticketSum, sigmaRef, bf){
  const scores=Array.from({length:12},(_,i)=>i+1).map(n=>({
    num:n, freq:allDraws.filter(d=>(d[bf]||[]).includes(n)).length,
    ritardo:(()=>{for(let i=allDraws.length-1;i>=0;i--){if((allDraws[i][bf]||[]).includes(n))return allDraws.length-1-i;}return allDraws.length;})(),
    score:calcBonusAffinita(n,allDraws,ticketSum,sigmaRef,bf),
  }));
...
function drawCanvas(canvas,series,frame,showMA5,hovered,W,H){
  ...
  if(hovered!==null&&hovered<visible){
    ...
    ctx.fillText(`Stelle: ${d.stelle?.join("-")||"—"}`,bx+10,by+80);
  }
  ...
  return(
    <div ref={containerRef}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:8,marginBottom:12}}>
        ...
        <KpiCard label="Σ Cinquina" value={cur.sum} color={cur.sum>MU_TEO?C.orange:C.teal} sub={`Δ=${cur.sum>MU_TEO?"+":""}${cur.sum-MU_TEO}`}/>
        ...
        <KpiCard label="Stelle" value={cur.stelle?.join(" · ")||"—"} color="#FFD700"/>
      </div>
      ...
      {/* ── GRAFICO DECINE ── */}
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:14,marginTop:14}}>
        <div style={{color:ACCENT,fontWeight:700,fontSize:13,marginBottom:4}}>🔢 Distribuzione per Decine (1–50)</div>
        <div style={{color:C.dim,fontSize:10,marginBottom:10}}>
            ...
            <div>
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={chartData} margin={{top:4,right:8,bottom:0,left:-20}}>
                  ...
                  {decine.map((dec,i)=>(
                    <Bar key={dec.label} dataKey={dec.label} stackId="b" fill={DC[i]} radius={i===4?[3,3,0,0]:0}/>
                  ))}
    ...
    <div>
      ...
      <div style={{marginTop:14,background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:14}}>
        ...
        <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12}}>
          <div style={{color:"#FFD700",fontWeight:700,fontSize:12,marginBottom:8}}>⭐ Frequenze Stelle (1–12)</div>
          {(()=>{
            const sf={};
            allDraws.forEach(d=>d.stelle?.forEach(s=>{sf[s]=(sf[s]||0)+1;}));
            ...
            return(
              <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                {Array.from({length:12},(_,i)=>i+1).map(n=>{
                  ...
                  const lastIdx=allRev.findIndex(d=>d.stelle?.includes(n));
                  ...
                  return(
                    <div key={n} style={{background:"#1a1a20",border:"1px solid #FFD70033",borderRadius:8,padding:"5px 8px",textAlign:"center"}}>
                      <Ball num={n} color="#FFD700" size={26} star/>
      ...
      <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:5}}>
        {bands.map(b=>(
          <div key={b.k} onClick={()=>setKBand(b.k)} style={{
            ...
            {b.k===1.5&&<span style={{color:ACCENT,fontSize:10}}>⭐</span>}
          </div>
        ))}
      </div>
    </div>
...
// TAB 4 — GENERATORE
// ═══════════════════════════════════════════════════════════════
function BonusAffinitaPanel({allDraws, ticketSum, sigmaRef, currentBonus, bonusField, label, selBonus, setSelBonus}){
  const suggestions = getBonusSuggestions(allDraws, ticketSum, sigmaRef, bonusField);
  const top8 = suggestions.slice(0,8);
  const selArr = selBonus.length>0 ? selBonus : (currentBonus||[]);
  const toggleSel = n => {
    setSelBonus(prev=>{
      if(prev.includes(n)) return prev.filter(x=>x!==n);
      if(prev.length>=2) return [...prev.slice(1),n];
  ...
  return(
    <div style={{background:"#0a0a10",border:"1px solid #FFD70033",borderRadius:10,padding:12,marginBottom:12}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>
        {top8.map((s,idx)=>{
          const isSel=selArr.includes(s.num);
          const barCol=idx===0?"#FFD700":idx<3?"#E8B84B":"#aaa";
          return(
            <div key={s.num} onClick={()=>toggleSel(s.num)} style={{
              background:isSel?"#FFD70022":"#0e0e1c",
              border:`2px solid ${isSel?"#FFD700":"#2a2a3a"}`,
              borderRadius:8,padding:"6px 7px",cursor:"pointer",textAlign:"center",
              boxShadow:isSel?"0 0 10px #FFD70044":"none",transition:"all 0.15s",
            }}>
              <Ball num={s.num} color={isSel?"#FFD700":"#888"} size={26} glow={isSel}/>
              ...
              <div style={{color:"#555",fontSize:7}}>{s.freq}x r.{s.ritardo}</div>
            </div>
          );
        })}
      </div>
      <div style={{background:"#FFD70008",border:"1px solid #FFD70033",borderRadius:8,padding:"8px 12px",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
        <span style={{color:"#4A4A6A",fontSize:11}}>{label} selezionati:</span>
        {selArr.length>0
          ? selArr.map(n=><Ball key={n} num={n} color="#FFD700" size={32} glow/>)
          : <span style={{color:"#555",fontSize:11}}>nessuno — clicca sopra</span>}
        {selArr.length>0&&<span style={{color:"#FFD700",fontFamily:"monospace",fontSize:11,marginLeft:4}}>
          Aff. media: {Math.round(selArr.reduce((s,n)=>{const f=suggestions.find(x=>x.num===n);return s+(f?.pct||0);},0)/selArr.length)}%
        </span>}
      </div>
    </div>
  );
...
function TabGeneratore(){
  ...
  const [manualStelle,setManualStelle]=useState(Array(STELLE_COUNT).fill(""));
  ...
  const [selEMBonus,setSelEMBonus]=useState([]);
  ...
  const generaTattico=()=>{
    ...
    setTimeout(()=>{
      ...
      while(found.length<5&&sc<maxAttempts){
        ...
        const st=generateStelle(sc+Date.now());
        found.push({nums,sum:s,evens,odds,stelle:st,zScore:zOf(s,MU_TEO,SIGMA_TEO).toFixed(2)});
      }
  ...
  return(
    <div>
      ...
      {mode==="auto"&&(
        <div>
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:10,alignItems:"center"}}>
            ...
            {[{id:"reale",l:`σ=${sigmaReale.toFixed(0)}`},{id:"teorica",l:"σ=30"}].map(s=>(
            ...
            <div style={{background:"#080816",border:`1px solid ${ACCENT}55`,borderRadius:12,padding:14}}>
              <div style={{display:"flex",justifyContent:"center",gap:10,flexWrap:"wrap",marginBottom:12}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:4}}>
                  ...
                  {stelle?.map(s=><Ball key={s} num={s} size={46} gold glow star/>)}
                  <span style={{color:"#FFD700",fontSize:9}}>⭐</span>
                </div>
              </div>

              <BonusAffinitaPanel allDraws={allDraws} ticketSum={ticket.sum} sigmaRef={sigmaEff} currentBonus={stelle} bonusField="stelle" label="Stelle" selBonus={selEMBonus} setSelBonus={setSelEMBonus}/>
              ...
              <button onClick={()=>{
                const t={id:Date.now(),nums:ticket.nums,stelle:selEMBonus.length===2?selEMBonus:stelle,
                ...
                localStorage.setItem(LS_TICKETS_EM,JSON.stringify([...prev,t]));
                alert(`✅ Cinquina salvata!\n${ticket.nums.join("-")} | Stelle: ${(selEMBonus.length===2?selEMBonus:(stelle||[])).join("-")||"—"}`);
      ...
      {mode==="manual"&&(
        <div>
          <div style={{color:C.dim,fontSize:11,marginBottom:10}}>Inserisci i {PICK} numeri + {STELLE_COUNT} Stelle (1–12).</div>
          <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap",marginBottom:10}}>
            ...
            <div style={{display:"flex",gap:4,alignItems:"center",marginLeft:8}}>
              <span style={{color:C.dim,fontSize:14}}>│</span>
              <div>
                <div style={{color:"#FFD700",fontSize:10,marginBottom:4}}>Stelle (1–12)</div>
                <div style={{display:"flex",gap:6}}>
                  {manualStelle.map((v,i)=>{
                    const num=parseInt(v)||0,valid=num>=1&&num<=STELLE_POOL;
                    return(
                      <div key={i} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                        <Ball num={valid?num:"?"} size={38} gold={valid} star={valid}/>
                        <input type="number" min={1} max={STELLE_POOL} value={v}
                          onChange={e=>{const next=[...manualStelle];next[i]=e.target.value;setManualStelle(next);}}
          ...
          <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:10}}>
            <button onClick={()=>{setManualInputs(Array(PICK).fill(""));setManualStelle(Array(STELLE_COUNT).fill(""));}} style={{
            ...
            <button onClick={()=>{
              ...
              setManualStelle(generateStelle(seed).map(String));
            }} style={{
      ...
      {mode==="tattico"&&(
        <div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:10,marginBottom:12}}>
            ...
            {/* FILTRO DECINE */}
            {(()=>{
              const DECINE_UI=[{label:"1–10",min:1,max:10},{label:"11–20",min:11,max:20},{label:"21–30",min:21,max:30},{label:"31–40",min:31,max:40},{label:"41–50",min:41,max:50}];
              const DC=["#E8B84B","#F07030","#8A5CC4","#4A8FD4","#2BA89A"];
              ...
              return(
                <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:12,gridColumn:"1 / -1"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <div style={{color:ACCENT,fontSize:11,fontWeight:700}}>🔢 Filtro Decine (1–50)</div>
                  ...
                  <div style={{color:C.dim,fontSize:9,marginBottom:8}}>
                    {decineAttive.size===0
                      ...
                      :`${decineAttive.size} decin${decineAttive.size===1?"a":"e"} attive — ${totRichiesti} numeri richiesti su 5`}
                  </div>
                  ...
                  <div style={{color:C.dim,fontSize:9,marginTop:8,lineHeight:1.5}}>
                    💡 <strong style={{color:ACCENT}}>+</strong> = aggiungi un numero da questa decina. Es. +2 sulla decina 21–30 = esattamente 2 numeri tra 21 e 30 nella cinquina generata.
                  </div>
                </div>
              );
              })()}
          ...
          {results.map((r,i)=>(
            <div key={i} style={{background:C.card,border:`1px solid ${ACCENT}33`,borderLeft:`3px solid ${ACCENT}`,borderRadius:9,padding:"10px 12px",marginBottom:8}}>
              ...
              <div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap"}}>
                ...
                <span style={{color:C.dim}}>│</span>
                <div style={{display:"flex",gap:5,alignItems:"center"}}>
                  {r.stelle.map(s=><Ball key={s} num={s} size={34} gold star/>)}
                  <span style={{color:"#FFD700",fontSize:10}}>Stelle</span>
                </div>
              </div>
            </div>
          ))}
...
function TabEstrazioni({ onUpdate }){
  ...
  const [stelle,setStelle]=useState(Array(STELLE_COUNT).fill(""));
  ...
  const add=()=>{
    ...
    const pStelle=stelle.map(v=>parseInt(v)||0);
    ...
    if(pStelle.some(x=>x<1||x>STELLE_POOL)){setError("Le Stelle devono essere tra 1 e 12");return;}
              ...
              <div>
                ...
                <div style={{display:"flex",gap:6}}>
                  {stelle.map((v,i)=>{
                    ...
                    return(
                      <div key={i} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                        <Ball num={valid?num:"?"} size={36} gold={valid} star={valid}/>
                        <input type="number" min={1} max={STELLE_POOL} value={v}
                          onChange={e=>{const n=[...stelle];n[i]=e.target.value;setStelle(n);}}
            ...
            {[...saved].reverse().map((d,i)=>{
              ...
              return(
                <div key={i} style={{display:"flex",alignItems:"center",gap:8,background:"#080816",border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 12px",flexWrap:"wrap"}}>
                  ...
                  {d.stelle?.map(st=><Ball key={st} num={st} size={26} gold star/>)}
                  <span style={{color:"#FFD700",fontSize:9}}>⭐</span>
...
function TabBiglietti(){
  ...
  function getResults(ticket){
    ...
    return allDraws.filter(d=>(d.n||0)>fromN).map(d=>{
      ...
      return {n:d.n,date:d.date,nums:d.nums,stelle:d.stelle,pts:matches.length,matches};
    });
  }
            ...
            <div key={ticket.id} style={{
              ...
              <div style={{padding:"12px 14px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",
                ...
                <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
                  ...
                  {ticket.stelle?.length&&<>
                    <span style={{color:C.dim,fontSize:14,alignSelf:"center"}}>│</span>
                    {ticket.stelle?.length&&<>{ticket.stelle.map(s=><Ball key={s} num={s} size={28} gold star/>)}</>}
                ...
                <div style={{borderTop:`1px solid ${C.border}`,padding:"12px 14px",background:"#06060e"}}>
                  <div style={{background:"#0a0a18",borderRadius:8,padding:"10px 12px",marginBottom:12,border:`1px solid ${ACCENT}33`}}>
                    <div style={{color:ACCENT,fontSize:11,fontWeight:700,marginBottom:6}}>
                      🎟 Cinquina giocata dopo conc.#{ticket.concorso||"?"}
                    </div>
...
export default function App(){
  ...
  const [extraDraws,setExtraDraws]=useState(()=>{
    try{return JSON.parse(localStorage.getItem(LS_KEY_EM)||"[]");}catch{return [];}
  });
      ...
      <div style={{maxWidth:780,margin:"0 auto",padding:"0 14px"}}>
        <div style={{background:"linear-gradient(180deg,#0c0c1e 0%,transparent 100%)",padding:"20px 0 0",textAlign:"center",marginBottom:0}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:10,marginBottom:6}}>
            ...
            <h1 style={{color:ACCENT,fontFamily:"Georgia,serif",fontSize:24,margin:0,textShadow:`0 0 30px ${ACCENT}44`}}>EuroMillions</h1>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:8,marginBottom:14,flexWrap:"wrap"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              <span style={{color:C.dim,fontSize:14}}>│</span>
              {last.stelle.map(s=><Ball key={s} num={s} size={34} gold star/>)}
              <span style={{color:"#FFD700",fontSize:9}}>⭐</span>
            </div>
          </div>
        </div>
        ...
        <div style={{marginTop:24,background:"#070712",border:"1px solid #111122",borderRadius:10,padding:12}}>
          <div style={{color:"#252535",fontSize:10,lineHeight:1.7}}>
            ⚠️ Conc.n.41 del 22/05/2026: 6-22-26-31-37 (Stelle 5-8). Jackpot {JACKPOT}. Strumento puramente statistico — nessun potere predittivo. Il gioco può causare dipendenza. Vietato ai minori di 18 anni.
          </div>
        </div>
      </div>
    </div>
