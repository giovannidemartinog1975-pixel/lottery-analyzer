...
const DRAWS = [
  ...
  { n:42, date:"23/05", nums:[5,34,35,42,46],  bonus:[3,5] },
];

const MU_TEO    = 127.5;
const SIGMA_TEO = 30;
const JACKPOT = "~120.000.000 €";
const ACCENT    = "#F07030";
const POOL      = 50;
const PICK      = 5;
const BONUS_POOL  = 12;
...
const LS_KEY_EJ = "draws_eurojackpot_v1";
const LS_TICKETS_EJ = "tickets_eurojackpot_v1";
...
function generateBonus(seed){
  ...
  const pool=Array.from({length:BONUS_POOL},(_,i)=>i+1);
  const picked=[];
...
function drawCanvas(canvas,series,frame,showMA5,hovered,W,H){
  ...
  if(hovered!==null&&hovered<visible){
    ...
    ctx.fillText(`Euro Numeri: ${d.bonus?.join("-")||"—"}`,bx+10,by+80);
  }
  ...
  return(
    <div ref={containerRef}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:8,marginBottom:12}}>
        ...
        <KpiCard label="Euro Numeri" value={cur.bonus?.join(" · ")||"—"} color="#FFD700"/>
      </div>
    ...
    <div>
      ...
      <div style={{marginTop:14,background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:14}}>
        ...
        <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12}}>
          <div style={{color:"#FFD700",fontWeight:700,fontSize:12,marginBottom:8}}>⭐ Frequenze Euro Numeri (1–12)</div>
          {(()=>{
            const sf={};
            allDraws.forEach(d=>d.bonus?.forEach(s=>{sf[s]=(sf[s]||0)+1;}));
            ...
            return(
              <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                {Array.from({length:12},(_,i)=>i+1).map(n=>{
                  ...
                  const lastIdx=allRev.findIndex(d=>d.bonus?.includes(n));
      ...
      <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:5}}>
        {bands.map(b=>(
          <div key={b.k} onClick={()=>setKBand(b.k)} style={{
            ...
            {b.k===1.5&&<span style={{color:ACCENT,fontSize:10}}>EN</span>}
          </div>
        ))}
      </div>
    </div>
...
function TabGeneratore(){
  ...
  const [manualBonus,setManualBonus]=useState(Array(BONUS_COUNT).fill(""));
  ...
  const [selEJBonus,setSelEJBonus]=useState([]);
  ...
  const generaTattico=()=>{
    ...
    setTimeout(()=>{
      ...
      while(found.length<5&&sc<maxAttempts){
        ...
        const st=generateBonus(sc+Date.now());
    ...
    <div>
      ...
      {mode==="auto"&&(
        <div>
          ...
          }}>🎲 Genera Cinquina + Euro Numeri</button>
          {ticket&&(
            <div style={{background:"#080816",border:`1px solid ${ACCENT}55`,borderRadius:12,padding:14}}>
              <div style={{display:"flex",justifyContent:"center",gap:10,flexWrap:"wrap",marginBottom:12}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:4}}>
                  ...
                  {bonus?.map(s=><Ball key={s} num={s} size={46} gold glow/>)}
                  <span style={{color:"#FFD700",fontSize:9}}>EN</span>
                </div>
              </div>

              <BonusAffinitaPanel allDraws={allDraws} ticketSum={ticket.sum} sigmaRef={sigmaEff} currentBonus={bonus} bonusField="bonus" label="Euro Numeri" selBonus={selEJBonus} setSelBonus={setSelEJBonus}/>
              ...
              <button onClick={()=>{
                const t={id:Date.now(),nums:ticket.nums,bonus:selEJBonus.length===2?selEJBonus:bonus,
                ...
                localStorage.setItem(LS_TICKETS_EJ,JSON.stringify([...prev,t]));
                alert(`✅ Cinquina salvata!\n${ticket.nums.join("-")} | Euro Numeri: ${(selEJBonus.length===2?selEJBonus:(bonus||[])).join("-")||"—"}`);
      ...
      {mode==="manual"&&(
        <div>
          <div style={{color:C.dim,fontSize:11,marginBottom:10}}>Inserisci i {PICK} numeri + {BONUS_COUNT} Euro Numeri (1–12).</div>
          <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap",marginBottom:10}}>
            ...
            <div style={{display:"flex",gap:4,alignItems:"center",marginLeft:8}}>
              <span style={{color:C.dim,fontSize:14}}>│</span>
              <div>
                <div style={{color:"#FFD700",fontSize:10,marginBottom:4}}>Euro Numeri (1–12)</div>
                <div style={{display:"flex",gap:6}}>
                  {manualBonus.map((v,i)=>{
                    const num=parseInt(v)||0,valid=num>=1&&num<=BONUS_POOL;
                    return(
                      <div key={i} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                        <Ball num={valid?num:"?"} size={38} gold={valid} />
                        <input type="number" min={1} max={BONUS_POOL} value={v}
          ...
          <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:10}}>
            <button onClick={()=>{setManualInputs(Array(PICK).fill(""));setManualBonus(Array(BONUS_COUNT).fill(""));}} style={{
            ...
            <button onClick={()=>{
              ...
              setManualBonus(generateBonus(seed).map(String));
            }} style={{
          ...
          {results.map((r,i)=>(
            <div key={i} style={{background:C.card,border:`1px solid ${ACCENT}33`,borderLeft:`3px solid ${ACCENT}`,borderRadius:9,padding:"10px 12px",marginBottom:8}}>
              ...
              <div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap"}}>
                ...
                <span style={{color:C.dim}}>│</span>
                <div style={{display:"flex",gap:5,alignItems:"center"}}>
                  {r.bonus.map(s=><Ball key={s} num={s} size={34} gold/>)}
                  <span style={{color:"#FFD700",fontSize:10}}>Euro Numeri</span>
                </div>
              </div>
            </div>
          ))}
...
function TabEstrazioni({ onUpdate }){
  ...
  const [bonus,setBonus]=useState(Array(BONUS_COUNT).fill(""));
  ...
  const add=()=>{
    ...
    const pBonus=bonus.map(v=>parseInt(v)||0);
    ...
    if(pBonus.some(x=>x<1||x>BONUS_POOL)){setError("Le Euro Numeri devono essere tra 1 e 12");return;}
                  ...
                  {bonus.map((v,i)=>{
                    ...
                    return(
                      <div key={i} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                        ...
                        <input type="number" min={1} max={BONUS_POOL} value={v}
                          onChange={e=>{const n=[...bonus];n[i]=e.target.value;setBonus(n);}}
            ...
            {[...saved].reverse().map((d,i)=>{
              ...
              return(
                <div key={i} style={{display:"flex",alignItems:"center",gap:8,background:"#080816",border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 12px",flexWrap:"wrap"}}>
                  ...
                  {d.bonus?.map(st=><Ball key={st} num={st} size={26} gold/>)}
                  <span style={{color:"#FFD700",fontSize:9}}>EN</span>
...
function TabBiglietti(){
  const allDraws=useDraws();
  const [tickets,setTickets]=useState(()=>{try{return JSON.parse(localStorage.getItem(LS_TICKETS_EJ)||"[]");}catch{return [];}});
  ...
  const persist=(list)=>{localStorage.setItem(LS_TICKETS_EJ,JSON.stringify(list));setTickets(list);};
  ...
  function getResults(ticket){
    ...
    return allDraws.filter(d=>(d.n||0)>fromN).map(d=>{
      ...
      return {n:d.n,date:d.date,nums:d.nums,bonus:d.bonus,pts:matches.length,matches};
    });
  }
              ...
              <div style={{padding:"12px 14px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",
                ...
                <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
                  ...
                  {ticket.bonus?.length&&<>
                    ...
                    {ticket.bonus?.length&&<>{ticket.bonus.map(b=><Ball key={b} num={b} size={28} gold/>)}</>}
                    <span style={{color:"#FFD700",fontSize:8}}>EN</span>
                  </>}
                </div>
...
export default function App(){
  ...
  const [extraDraws,setExtraDraws]=useState(()=>{
    try{return JSON.parse(localStorage.getItem(LS_KEY_EJ)||"[]");}catch{return [];}
  });
      ...
      <div style={{maxWidth:780,margin:"0 auto",padding:"0 14px"}}>
        <div style={{background:"linear-gradient(180deg,#0c0c1e 0%,transparent 100%)",padding:"20px 0 0",textAlign:"center",marginBottom:0}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:10,marginBottom:6}}>
            ...
            <h1 style={{color:ACCENT,fontFamily:"Georgia,serif",fontSize:24,margin:0,textShadow:`0 0 30px ${ACCENT}44`}}>EuroJackpot</h1>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:8,marginBottom:14,flexWrap:"wrap"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              <span style={{color:C.dim,fontSize:14}}>│</span>
              {last.bonus.map(s=><Ball key={s} num={s} size={34} gold/>)}
              <span style={{color:"#FFD700",fontSize:9}}>EN</span>
            </div>
          </div>
        </div>
        ...
        <div style={{marginTop:24,background:"#070712",border:"1px solid #111122",borderRadius:10,padding:12}}>
          <div style={{color:"#252535",fontSize:10,lineHeight:1.7}}>
            ⚠️ Conc.n.42 del 23/05/2026: 5-34-35-42-45 (EN 3-5). Jackpot {JACKPOT}. Strumento puramente statistico — nessun potere predittivo. Il gioco può causare dipendenza. Vietato ai minori di 18 anni.
          </div>
        </div>
      </div>
    </div>
