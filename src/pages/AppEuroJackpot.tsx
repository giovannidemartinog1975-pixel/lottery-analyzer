...
function TabEstrazioni({onUpdate}){
  ...
  const add=async()=>{
    ...
    try{if(allDraws.length>=6){const lstm=computeLSTMEJ(allDraws);const reg=computeRegressionEJ(allDraws);const combined=Math.round(lstm.predictedSum*0.40+reg.predicted*0.35+reg.wma*0.25);const zPred=zOf(combined,MU_TEO,SIGMA_TEO);await supabase.from("predizioni").insert({lotteria:"eurojackpot",concorso:n,dadata_predizione:date.trim().split("/").length===2?`2026-${date.trim().split("/")[1].padStart(2,"0")}-${date.trim().split("/")[0].padStart(2,"0")}`:date.trim(),somma_predetta:combined,somma_reale:sm(newDraw.nums),lstm:lstm.predictedSum,regressione:reg.predicted,wma:Math.round(reg.wma),trend:lstm.currentTrend,z_predetto:parseFloat(zPred.toFixed(3)),scarto:sm(newDraw.nums)-combined});}}catch(err){console.error("Predizione EJ save error:",err);}tadata_predizione:date.trim().split("/").length===2?`2026-${date.trim().split("/")[1].padStart(2,"0")}-${date.trim().split("/")[0].padStart(2,"0")}`:date.trim(),somma_predetta:combined,somma_reale:sm(newDraw.nums),lstm:lstm.predictedSum,regressione:reg.predicted,wma:Math.round(reg.wma),trend:lstm.currentTrend,z_predetto:parseFloat(zPred.toFixed(3)),scarto:sm(newDraw.nums)-combined});}}catch(err){console.error("Predizione EJ save error:",err);}_predizione:dateIso,somma_predetta:combined,somma_reale:sm(newDraw.nums),lstm:lstm.predictedSum,regressione:reg.predicted,wma:Math.round(reg.wma),trend:lstm.currentTrend,z_predetto:parseFloat(zPred.toFixed(3)),scarto:sm(newDraw.nums)-combined});}}catch(err){console.error("Predizione EJ save error:",err);}
      ...
      setTimeout(()=>{
        ...
        setTimeout(()=>{
          const scored=allCandidates.map(c=>{
            ...
            const topBonus=bonusAffinita.slice(0,2).map(b=>b.num);
            return {...c,sum:s,total:parseFloat(total.toFixed(1)),advS:parseFloat(advS.toFixed(1)),ensS:parseFloat(ensS.toFixed(1)),pairS:parseFloat(pairS.toFixed(1)),distS:parseFloat(distS.toFixed(1)),zoneS,zScore:zOf(s,MU_TEO,SIGMA_TEO).toFixed(2),evens,odds:PICK-evens,ritMedio,pairBonus:parseFloat(pairB.toFixed(2)),topBonus};
          });
      ...
      <div style={{background:"#1a0e00",border:`1px solid ${GEN_COLOR}44`,borderRadius:12,padding:14,marginBottom:14}}>
        ...
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {[{l:"🧬 Avanzato",v:wAdv,set:setWAdv,c:"#22d3ee"},{l:"🔬 Predittivo",v:wEns,set:setWEns,c:"#e879f9"},{l:"🔗 Coppie",v:wPair,set:setWPair,c:C.orange},{l:"📐 Somma",v:wDist,set:setWDist,c:C.teal},{l:"🌡️ Zone",v:wZone,set:setWZone,c:"#f472b6"}].map(row=>(
              <div key={row.l}>
...
function TabPerformance(){
  ...
  useEffect(()=>{
    async function load(){
      try{
        const{data,error}=await supabase.from("predizioni").select("*").eq("lotteria","eurojackpot").order("concorso",{ascending:false}).limit(50);
        if(error)throw error;
        setStorico(data||[]);
