...
function TabBanda(){
  ...
  const predizione=useMemo(()=>{
    ...
    return{combined,lstm:lstm.predictedSum,reg:reg.predicted,wma:Math.round(reg.wma),zPred:parseFloat(zPred.toFixed(2)),lo:combined-confidence,hi:combined+confidence,trend:lstm.currentTrend};
  },[allDraws,sigmaReale]);
  useEffect(()=>{
              ...
              {matches.length===0?(
                ...
                <>
                  ...
                  <div style={{color:C.dim,fontSize:9,lineHeight:1.7}}>
                    Codifica: <span style={{color:C.red}}>++</span> sopra +1σ · <span style={{color:C.orange}}>+</span> sopra media · <span style={{color:C.teal}}>-</span> sotto media · <span style={{color:C.blue}}>--</span> sotto -1σ. Solo informativo — non concorre allo score Unificato.
                  </div>
                </>
              )}
            </div>
  ...
  return (
    <div>
      ...
      {(()=>{
        ...
        // Range consigliato da predizione
        const lstm=computeLSTM(allDraws);
        const reg=computeRegression(allDraws);
        ...
        const pdConsigliato=avgPD>=3.5?"4P–2D":avgPD>=2.5?"3P–3D":"2P–4D";
        return(
...
function TabPerformance(){
  ...
  useEffect(()=>{
    async function load(){
      try{
        const{data,error}=await supabase.from("predizioni").select("*").eq("lotteria","superenalotto").order("concorso",{ascending:false}).limit(50);
        if(error)throw error;
        setStorico(data||[]);
  ...
  return(
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Courier New',monospace",color:C.text,paddingBottom:60}}>
      <div style={{maxWidth:780,margin:"0 auto",padding:"0 12px"}}><div style={{position:"sticky",top:0,zIndex:100,background:C.bg}}>
        ...
        <div style={{display:tab==="wheeling"?"block":"none"}}><TabPoolWheeling gioco="SE" estrazioni={allDraws.map(d=>({data:d.dataISO||"",nums:d.nums,jolly:d.jolly,superstar:d.superstar}))} onSalvaBiglietto={salvaBigliettoOracolo} bigliettiSalvati={bigliettiOracolo}/></div>
